...
# dependencies out of it. None yet: guard and classify are packages of
# the root module.
SUBMODULES =
...
# The module sits above the loop and must build from openresponses,
# agenttool, agentturn and the standard library alone.
deps:
	@deps=$$($(GO) list -deps -f '{{if not .Standard}}{{.ImportPath}}{{end}}' ./... | grep -v '^github.com/ChristopherDavenport/agentpolicy' | grep -v '^github.com/ChristopherDavenport/openresponses' | grep -v '^github.com/ChristopherDavenport/agenttool' | grep -v '^github.com/ChristopherDavenport/agentturn' || true); \
